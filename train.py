...
def train(opt):
    ...
    gpu_info()  
    save_path, best_ckp_save_path, best_ckp_file, result_save_path, _ = check_dirs()
    ...
    for epoch in range(opt.epochs):
        ...
        for i, (batch_img1, batch_img2, batch_label1, batch_label2, _) in enumerate(train_tbar):
            ...
            loss_seg = criterion(outs, (batch_label1, batch_label2)) if model.dl else criterion(outs, (batch_label1,))
            ...
            target_loss = F.interpolate(batch_label, size=(14,14), mode='bilinear', align_corners=True)  #8 1 64 64
            loss2 = loss_Contrastive(outputs,target_loss.cuda())

            loss = loss_seg + 0.2 * loss2
